...
for file in filelist:
    ...
    capacity_arr = capacity_arr[capacity_arr >= 5]  # same cutoff as in training

    split_index = np.searchsorted(capacity_arr, x0)

    def linear_eval(x): return a * x + b
    def exp_eval(x): return c * (np.exp(d * x) - np.exp(d * x0))

    voltage_pred = np.zeros_like(capacity_arr)
    voltage_pred[:split_index] = linear_eval(capacity_arr[:split_index])
    voltage_pred[split_index:] = linear_eval(capacity_arr[split_index:]) + exp_eval(capacity_arr[split_index:])
